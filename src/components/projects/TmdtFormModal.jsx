...
export default function TmdtFormModal({ 
  ...
  return createPortal(
    ...
    <div className="fixed inset-0 z-[9999] flex justify-center items-start pt-6 pb-6 px-4 bg-background/80 backdrop-blur-sm animate-fade-in overflow-hidden">
      {/* Main Modal Window */}
      <div className="bg-card border border-border rounded-3xl max-w-xl w-full flex flex-col max-h-[calc(100vh-48px)] shadow-2xl overflow-hidden">
        {/* Modal Header (Fixed) */}
        <div className="px-6 py-3.5 bg-muted/90 border-b border-border/80 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-success/20 border border-success/30 flex items-center justify-center text-success font-bold font-mono text-sm">
              Lần {phaseNumber}
            </div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          {/* Scrollable Body */}
          <div className="p-5 space-y-3.5 flex-1 overflow-y-auto min-h-0 text-xs">
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-foreground/80 mb-1 flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 text-primary" /> Ngày Phê Duyệt / Quyết Định <span className="text-destructive">*</span>
              </label>
              <input
                type="date"
                value={formData.date}
            ...
            <div>
              <label className="block text-xs font-semibold text-foreground/80 mb-1 flex items-center gap-1">
                <FileText className="w-3.5 h-3.5 text-primary" /> Số Quyết Định Phê Duyệt
              </label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground/80 mb-1">
              Nội Dung Phê Duyệt / Điều Chỉnh <span className="text-destructive">*</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground/80 mb-1 flex items-center gap-1">
              <DollarSign className="w-3.5 h-3.5 text-success" /> Tổng Mức Đầu Tư Sau Điều Chỉnh (VNĐ) <span className="text-destructive">*</span>
            </label>
            ...
            {newAmountNum > 0 && (
              <div className="mt-3 p-3.5 rounded-xl bg-muted/80 border border-border space-y-1.5 text-xs">
                {prevAmount > 0 && (
                  <div className="flex justify-between items-center text-foreground/80">
                    <span>TMĐT trước điều chỉnh (Lần {phaseNumber - 1}):</span>
                    <span className="font-mono font-semibold text-foreground">{formatVND(prevAmount)}</span>
                  </div>
                )}
                <div className="flex justify-between items-center text-foreground font-medium">
                  <span>TMĐT sau điều chỉnh (Lần {phaseNumber}):</span>
                  <span className="font-mono font-bold text-success">{formatVND(newAmountNum)}</span>
                </div>
                {prevAmount > 0 && (
                  <div className="flex justify-between items-center pt-1 border-t border-border/60 font-mono">
                    <span className="text-muted-foreground">Giá trị tăng / giảm:</span>
                    <span className={`font-bold ${diffAmount >= 0 ? 'text-primary' : 'text-destructive'}`}>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2 bg-muted border border-border rounded-xl text-sm text-foreground focus:outline-none focus:border-primary transition"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground/80 mb-1 flex items-center gap-1">
              <Paperclip className="w-3.5 h-3.5 text-primary" /> Tệp Quyết Định Phê Duyệt (Tên file / link)
            </label>
            <input
              type="text"
              ...
              className="w-full px-3.5 py-2 bg-muted border border-border rounded-xl text-sm text-foreground focus:outline-none focus:border-primary font-mono transition"
            />
          </div>
          </div>
          ...
          <div className="px-6 py-3.5 bg-card border-t border-border flex items-center justify-end gap-3 shrink-0">
            ...
            <button
              type="submit"
              className="px-6 py-2 rounded-xl bg-success hover:bg-success text-foreground text-xs font-semibold shadow-lg shadow-emerald-600/30 transition cursor-pointer"
            >
